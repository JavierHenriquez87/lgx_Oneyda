...
async function dataSaldoIngresosGeneral({
  datobusqueda = null,
  tipo_documento = null,
  limit = 50,
  offset = 0,
  ...
  return { data: general, total };
}
async function dataSaldoIngresosDetalle({ iga_stal_id, iga_id }) {
  let detalles = [];
  let saldoInicial = [];
  let salidas = [];
  let salidasdetalles = [];
  ...
  if (iga_stal_id != 1) {
    const detalleSQL = `
      SELECT
        ...
        dm
      FROM (
        SELECT i.iga_id AS idiga, i.iga_codigo AS nInforme
        FROM clg_iga_informeguardalmacen AS i
        WHERE i.iga_id = ${iga_id} AND i.iga_com_id = 38
      ) iga
      LEFT JOIN (
        SELECT
          d.dga_id AS iddga,
      ...
      LEFT JOIN (
        SELECT doc.doc_iga_id AS idigadoc, doc.doc_ndocumento AS dm
    ...
    const detallesalida = `
      ...
      WHERE sal_iga_id = ${iga_id}
      ORDER BY sal_fecha ASC;
    `;

    const [resDetalles] = await db.query(detalleSQL);
    const [resInicial] = await db.query(detalleinicial);
    const [resSalidas] = await db.query(detallesalida);

    detalles = resDetalles;
    saldoInicial = resInicial;
    salidas = resSalidas;
    ...
    for (const salida of salidas) {
      ...
      `;
      const [resDetalleSalida] = await db.query(detalleSalidaSQL);
      salidasdetalles.push(...resDetalleSalida);
    }
  }
...
module.exports = {
  dataSaldoIngresosGeneral,
  dataSaldoIngresosDetalle,
