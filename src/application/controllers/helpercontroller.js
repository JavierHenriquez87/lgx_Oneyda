...
const CortFacturaCorrelativo = require("../models/CortFacturaCorrelativoModel");
const {
  data_consolidado,
} = require("../services/ObtenerDatosConsolidadoService");
...
class helpercontroller {
  ...
  static async CrearPDFSALConsolidada({ req, res, next, id }) {
    ...
    try {
      ...
      const html = await new Promise((resolve, reject) => {
        res.render(
          "formatoSALConsolidada",
          {
            ...
            fechaImpresion: moment
              .utc(infoSalSalida.datos.general.fecha)
              .format("DD-MM-YYYY hh:mm A"),
            fechadm: moment
              .utc(infoSalSalida.datos.general.fechadm)
              .format("DD-MM-YYYY"),
            fecha: moment
              .utc(infoSalSalida.datos.general.fecha)
              .format("DD-MM-YYYY"),
      ...
      const footerHtml = await new Promise((resolve, reject) => {
        res.render(
          "footerSAL",
          { endoso: infoSalSalida.datos.general.endosado },
          (err, footerHtml) => {
            if (err) reject(err);
            resolve(footerHtml);
          }
  ...
  /**
   * TODO FUNCIÓN PARA CREAR Y GUARDAR PDF DE FACTURA
   */
  static async CrearPDFactura({ req, res, next, factura_id }) {
    //creamos el dato para subirlo a DO Space
    const do_path = process.env.DO_SPACES_PATH;
    const do_spe = process.env.DO_SPACES_ENDPOINT || "";
    const do_spk = process.env.DO_SPACES_KEY;
    ...
    try {
      const detalle_factura = await Facturas.findAll({
        where: {
          fact_id: factura_id,
        },
      ...
      // Nombre del archivo
      const filename = datos_carta.cac_numero + ".pdf";
      path_carta = do_path + "Factura/" + filename;

      // Configurar el endpoint de DigitalOcean Spaces
      const spacesEndpoint = new AWS.Endpoint(do_spe + "Factura/");
      // Configurar S3 con las credenciales de DigitalOcean Spaces
      const s3 = new AWS.S3({
        endpoint: spacesEndpoint,
        accessKeyId: do_spk,
