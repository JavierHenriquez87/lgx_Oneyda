...
class informesguardaalmacencontroller {
                ...
                dataInforme = dataInforme.map((informe) => {
                    return {
                        ...
                        archivo: informe.de_lgx
                            ? informe.iga_archivo
                            : 'https://sistemas.clgsv.com/ucontrol/ci/clg/pdf/informes_guardalmacen/' + informe.iga_archivo
                    };
                });
            }
    ...
    static async ObtInfoIGA(req, res, next) {
        ...
        try {
            ...
            if (Array.isArray(dataInforme) && dataInforme.length > 0) {
                dataInforme = dataInforme.map((informe) => {
                    const info = typeof informe.toJSON === 'function' ? informe.toJSON() : informe;

                    return {
                        ...info,
                        iga_archivo: info.iga_archivo && !info.iga_archivo.startsWith('http')
                            ? 'https://sistemas.clgsv.com/ucontrol/ci/clg/pdf/informes_guardalmacen/' + info.iga_archivo
                            : info.iga_archivo,
                        iga_archivo_bl: info.iga_archivo_bl && !info.iga_archivo_bl.startsWith('http')
                            ? 'https://sistemas.clgsv.com/ucontrol/ci/clg/pdf/informes_guardalmacen/' + info.iga_archivo_bl
                            : info.iga_archivo_bl,
                        iga_archivo_dmti: info.iga_archivo_dmti && !info.iga_archivo_dmti.startsWith('http')
                            ? 'https://sistemas.clgsv.com/ucontrol/ci/clg/pdf/informes_guardalmacen/' + info.iga_archivo_dmti
    ...
    static async ObtenerVinetas(req, res, next) {
        ...
        try {
            ...
            for (const data of vinetas) {
                if (data.de_lgx == 1) {
                    data.archivo = 'https://wms-assets.nyc3.digitaloceanspaces.com/LGX/Vinetas/' + data.tar_identificador + '.pdf';
                } else {
                    data.archivo = 'https://sistemas.clgsv.com/ucontrol/ci/clg/pdf/vinetas/' + data.tar_identificador + '.pdf';
                }
            }
        ...
        try {
            ...
            if (vinetas.length == 0) {
                jsonResponse = {
                    status: 404,
                    message: "Not Found",
                    response: "No se encontraron registros para la creación de las viñetas."
                };
    ...
    */
    static async EditarComentarios(req, res, next) {
        ...
        const validaciones = [
            check('iga_id').notEmpty().withMessage('iga_id is required.'),
            check('iga_comentarios').notEmpty().withMessage('iga_comentarios is required.')
        ];
        ...
        try {
            ...
            await InformeGuardaAlmancen.update(
                {
                    iga_comentarios
                },
                {
    ...
    */
    static async auditoriaDocumentos(req, res, next) {
        ...
        const validaciones = [
            ...
            check('tipo_regimen').notEmpty().withMessage('tipo_regimen es requerido.')
        ];
        ...
        try {
            ...
            req.body.datobusqueda = req.body.datobusqueda ?? null;
            //* Guardamos en estas variables la data que envió el usuario
            const { page, fecha_inicio, fecha_fin, tipo_regimen, datobusqueda, tipobusqueda } = req.body;
            let documentos = [];
            ...
            const fechaInicio = moment(fecha_inicio).startOf('day').format('YYYY-MM-DD HH:mm:ss');
            const fechaFin = moment(fecha_fin).endOf('day').format('YYYY-MM-DD HH:mm:ss');
            ...
            for (const docs of documentos) {
                for (const sal of docs.clg_sal_salidas) {
                    for (const doc_docs of sal.clg_docs_documentos) {
                        doc_docs.dataValues.docs_file_path = 'https://sistemas.clgsv.com/ucontrol/ci/clg/documentos/' + sal.sal_id + '/' + doc_docs.docs_file;
                    }
                }

                for (const im of docs.clg_doc_documentos) {
                    im.dataValues.docs_file_path = 'https://sistemas.clgsv.com/ucontrol/ci/clg/cambioregimen/' + docs.iga_id + '/' + im.doc_documentos;
                }
            }
    ...
    static async ObtInfoParaCrearInformes(req, res, next) {
        ...
        try {
                    ...
                    },
                    {
                        ...
                        include: [
                            ...
                            },
                            {
                                ...
                                attributes: ['iga_archivo', 'iga_archivo_bl', 'iga_archivo_dmti']
                            }
                        ]
                    }
                ]
            ...
            if (dataDetalleCA) {
                ...
                dataInfoCarta.dataValues.iga_fechaingreso = dataInformeGA.iga_fechaingreso;
                dataInfoCarta.dataValues.iga_fechaabandono = dataInformeGA.iga_fechaabandono;
                ...
                dataInfoCarta.dataValues.iga_codtransportista = dataInformeGA.iga_codtransportista;
                dataInfoCarta.dataValues.iga_placavehiculo = dataInformeGA.iga_placavehiculo;
    ...
    */
    static async CrearInformesGuardaAlmacen(req, res, next) {
        ...
        const validaciones = [
            ...
            check('iga_marchamo').notEmpty().withMessage('iga_marchamo es requerido.'),
            check('iga_codtransportista').notEmpty().withMessage('iga_codtransportista es requerido.'),
            check('iga_placavehiculo').notEmpty().withMessage('iga_placavehiculo es requerido.'),
            check('iga_manifiesto').notEmpty().withMessage('iga_manifiesto es requerido.'),
            check('iga_fechaabandono').notEmpty().withMessage('iga_fechaabandono es requerido.'),
            check('iga_fechaingreso').notEmpty().withMessage('iga_fechaingreso es requerido.'),
            check('iga_pai_id').notEmpty().withMessage('iga_pai_id es requerido.'),
            check('iga_ubicacion').notEmpty().withMessage('iga_ubicacion es requerido.'),
        ...
        try {
            ...
            for (const [i, detalle] of detalles.entries()) {
                ...
                const correlativo = await informesguardaalmacencontroller.ObtenerCorrelativoIga();
                ...
                if (validarCarta.misma_dmti == null || misma_dmti == 0) {
                    ...
                    const extensionDMTI = await helpercontroller.getExtensionFromMimeType(archivoDMTI.mimetype);
                ...
                //* Guardar archivo BL
                const extensionBL = await helpercontroller.getExtensionFromMimeType(archivoBL.mimetype);
                ...
                saveIGA = await InformeGuardaAlmacen.create({
                    ...
                    iga_fechaabandono,
                    iga_fechaingreso,
                    iga_dmti: detalle.iga_dmti != null ? detalle.iga_dmti.toUpperCase() : null,
                    iga_pai_id,
                ...
                saveDGA = await DetalleGuardaAlmancen.create({
                    ...
                    dga_peso_neto: detalle.dga_peso_neto == '' ? 0 : detalle.dga_peso_neto,
                    dga_peso: detalle.dga_peso == '' ? 0 : detalle.dga_peso,
                    dga_volumen: detalle.iga_volumen == '' ? 0 : detalle.iga_volumen,
                    dga_volumen_fisico: detalle.iga_volumen_fisico == '' ? 0 : detalle.iga_volumen_fisico,
                    ...
                    dga_fecha_exp: detalle.dga_fecha_exp == '' ? '0000-00-00 00:00:00' : detalle.dga_fecha_exp,
                ...
                observacion = await informesguardaalmacencontroller.CrearPdfIga(req, res, next, saveIGA.iga_id);
    ...
    */
    static async EditarInformesGuardaAlmacen(req, res, next) {
        ...
        const validaciones = [
            ...
            check('iga_doctransporte').notEmpty().withMessage('iga_doctransporte es requerido.'),
            check('iga_comentarios').notEmpty().withMessage('iga_comentarios es requerido.'),
            check('iga_sobredimensionado').notEmpty().withMessage('iga_sobredimensionado es requerido.'),
            check('iga_bultos_sd').notEmpty().withMessage('iga_bultos_sd es requerido.'),
            check('iga_volumen_sd').notEmpty().withMessage('iga_volumen_sd es requerido.'),
            check('iga_menaje_casa').notEmpty().withMessage('iga_menaje_casa es requerido.'),
            check('iga_tarimas_ocupadas').notEmpty().withMessage('iga_tarimas_ocupadas es requerido.')
        ];
        ...
        try {
            ...
            if (oldData.iga_estado == 0 || oldData.iga_salida == 1) {
                jsonResponse = {
                    status: 400,
                    message: "Error",
                    response: "No puedes modificar este informe. Se encuentra Inactivo o ya tiene salidas."
                };
            ...
            const archivoDMTI = req.files['iga_archivo_dmti'] ? req.files['iga_archivo_dmti'][0] : null;
            const archivoBL = req.files['iga_archivo_bl'] ? req.files['iga_archivo_bl'][0] : null;
            let pathDMTI = null;
            let pathBL = null;
            ...
            if (pathBL != null) {            // null o undefined → NO entra
            ...
            for (const det of detalles) {
                ...
                if (det.dga_id == 0) {
                    ...
                    saveDGA = await DetalleGuardaAlmancen.create({
                        ...
                        dga_volumen_fisico: det.iga_volumen_fisico == '' ? 0 : det.iga_volumen_fisico,
                        ...
                        dga_fecha_exp: det.dga_fecha_exp == '' ? '0000-00-00 00:00:00' : det.dga_fecha_exp,
                ...
                //TODO CREAR ARCHIVO PDF
                observacion = await informesguardaalmacencontroller.CrearPdfIga(req, res, next, iga_id);
            ...
            jsonResponse = {
                status: 400,
                message: "Error",
                response: `Ocurrio un error al editar el Informe Guarda Almacen. ==> ${error.message}`
            };
    ...
    static async CrearPdfIga(req, res, next, iga_id_param = null) {
        ...
        try {
            ...
            const html = await new Promise((resolve, reject) => {
                res.render('formatoIGA', {
                    iga_codigo: infoIga.iga_codigo,
                    iga_fecha_recepcionado: moment(infoIga.iga_fecha_recepcionado).format('DD-MM-YYYY hh:mm A'),
                    stal_estado: infoIga.clg_stal_estadosalmacenaje.stal_estado,
                    iga_fechaingreso: infoIga.iga_fechaingreso,
                    tra_nombre: infoIga.clg_cac_aceptacion.clg_tra_transportista.tra_nombre,
                    ...
                    pai_nombre_destino: infoIga.clg_cad_detalleaceptacion.clg_pai_paise.pai_nombre,
            ...
            const path_carta = do_path + 'DocumentosIGA/informes_guardaalmacen/' + filename;
            //* Configurar el endpoint de DigitalOcean Spaces
            const spacesEndpoint = new AWS.Endpoint(do_spe + 'DocumentosIGA/informes_guardaalmacen/');
            //* Configurar S3 con las credenciales de DigitalOcean Spaces
            const s3 = new AWS.S3({
                endpoint: spacesEndpoint,
                accessKeyId: do_spk,
    ...
    */
    static async CrearIGACambioRegimen(req, res, next) {
        ...
        // Definir las validaciones de campos dentro del método
        const validaciones = [
            check('iga_id').notEmpty().withMessage('iga_cac_id es requerido.'),
        ...
        try {
            ...
            const codigocorrelativo = correlativoActual.replace(/[A-Z]+/, infoEstadoAlmacenaje.stal_codsistema);
